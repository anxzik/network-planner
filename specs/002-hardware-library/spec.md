...
- **FR-005**: When an appliance type is deleted or edited, topologies that
  already place it MUST remain openable and MUST NOT lose their placed
  appliances.
...
- **FR-010**: An import MUST apply completely or not at all; a failure MUST
  leave the library as it was.
- **FR-011**: When an import fails, the application MUST say what was wrong with
  the file.
- **FR-012**: A library file MUST record the format version it was written in.
- **FR-013**: The application MUST refuse a library file written in a format
  version it does not understand, and MUST say so, rather than importing part of
  it.
...
- **FR-020**: An appliance type MUST be able to have no ports.
...
- **Where**: the library persists independently of any single topology, so that
  it is available across every plan the person makes. It is not stored inside a
  project file.
...
- **SC-003**: No import can leave the library in a partly-updated state.
- **SC-004**: All 131 appliance types available before this feature are still
  available after it, with the same ports and symbols.
- **SC-005**: Opening a topology that uses a deleted or edited appliance type
  never loses placed appliances.
...
- **SC-007**: Application start is not noticeably slower with a library of
  several hundred types than with the shipped 131.
